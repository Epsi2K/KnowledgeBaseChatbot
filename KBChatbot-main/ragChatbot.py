# qwen_chatbot.py (CPU-only, RAG enabled, no quantization)
...
def chat_with_qwen(user_input, history=[]):
    ...
    context_text = "\n---\n".join(context_chunks[:3])  # separate chunks visually

    # Build stronger system prompt with explicit context tag
    system_message = (
        f"<|im_start|>system\nYou are a knowledgeable assistant. Answer the user's question using only the following context."
        f"\nContext:\n{context_text}\n<|im_end|>"
    )
